...
def main_nortrip_loop(
    *,
    ...
    Args:
        time_config: Time configuration (min/max times, dt, etc.).
        converted_data: Converted input datasets.
        metadata_input: Metadata input configuration.
        initial_input: Initial state inputs.
        model_flags: Model flags/toggles (forecast, OSPM, save options, etc.).
        model_parameters: Model parameterization, including num_track.
        meteorology_input: Meteorology input data and flags.
        traffic_input: Traffic input (dates, strings, etc.).
        activity_input: Activity input data.
        airquality_input: Air quality input data.
        model_activities: Activity configuration/rules.

    Returns:
        model_variables: Model variables after the loop completes.
    """
