...
def read_input_airquality(
    ...
    col_idx = find_column_index("EP_emis", header_text)
    EP_emis_available = 0
    if col_idx != -1:
    ...
    col_idx = find_column_index("Salt_obs(na)", header_text)
    ...
    col_idx = find_column_index("Disp_fac", header_text)
    f_dis_available = 0
    if col_idx != -1:
    ...
    if ospm_df is not None and not ospm_df.empty:
        ...
        col_idx = find_column_index("FFospm(m/s)", ospm_header_text)
        if col_idx != -1:
            U_mast_ospm_orig[:] = (
                apply_safe_float(ospm_working_df, col_idx, nodata)
            )

        col_idx = find_column_index("DDospm(deg)", ospm_header_text)
        if col_idx != -1:
            wind_dir_ospm_orig[:] = (
                apply_safe_float(ospm_working_df, col_idx, nodata)
            )

        col_idx = find_column_index("TKospm(degK)", ospm_header_text)
        if col_idx != -1:
            TK_ospm_orig[:] = apply_safe_float(ospm_working_df, col_idx, nodata)

        col_idx = find_column_index(
            "Globalradiationospm(W/m^2)", ospm_header_text
        )
        if col_idx != -1:
            GlobalRad_ospm_orig[:] = (
                apply_safe_float(ospm_working_df, col_idx, nodata)
        ...
        col_idx = find_column_index(
            "Cbackgroundospm(ug/m^3)", ospm_header_text
        )
        if col_idx != -1:
            cNOx_b_ospm_orig[:] = (
                apply_safe_float(ospm_working_df, col_idx, nodata)
