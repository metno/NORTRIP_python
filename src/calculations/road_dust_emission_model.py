...
def road_dust_emission_model(
    ...
    for s in range(constants.num_wear):
        ...
        for t in range(constants.num_tyre):
            for v in range(constants.num_veh):
                ...
                snow_ice_sum = np.sum(model_variables.g_road_data[constants.snow_ice_index, ti, tr, ro])
    ...
    # --------------------------------------------------------------------------
    f_PM_adjust = np.ones((constants.num_source, constants.num_size, constants.num_tyre, constants.num_veh))
    s = constants.road_index
    x_indices = [constants.pm_10, constants.pm_25]
    
    for v in range(constants.num_veh):
        # Only allow the parameterisation between 20 and 60 km/hr
        V_temp = min(60, max(20, converted_data.traffic_data[constants.V_veh_index[v], ti, ro]))
        for x in x_indices:
            f_PM_adjust[s, x, :, v] = (
                (1 + model_parameters.c_pm_fraction * V_temp) /
    ...
    for s in range(constants.num_wear):
        ...
        for t in range(constants.num_tyre):
            for v in range(constants.num_veh):
                P_wear[:] += (
                    WR_array[s, t, v] *
                    (1 - model_parameters.f_0_dir[s] * model_variables.f_q[s, ti, tr, ro]) *
        ...
        model_variables.E_road_bin_data[s, :, constants.E_direct_index, ti, tr, ro] = E_wear[:]
        
        # Total retained wear distributed over all tracks according to area
        for tr2 in range(model_parameters.num_track):
            model_variables.M_road_bin_balance_data[s, :, constants.P_wear_index, ti, tr2, ro] = (
    ...
    if model_flags.abrasion_flag:
        ...
        for s in range(constants.num_source_all):
            if s < len(model_parameters.p_0_abrasion) and model_parameters.p_0_abrasion[s] > 0:
                for t in range(constants.num_tyre):
                    for v in range(constants.num_veh):
                        snow_ice_sum = np.sum(model_variables.g_road_data[constants.snow_ice_index, ti, tr, ro])
                        ...
                        abrasion_temp[:] = (
                            converted_data.traffic_data[constants.N_t_v_index[(t, v)], ti, ro] /
                            metadata.n_lanes * model_parameters.veh_track[tr] *
                            f_abrasion_temp[:] * M_road_bin_0_data[s, :]
                        )
                        
                        P_abrasion[:] += abrasion_temp[:] * (1 - model_parameters.f_0_dir[constants.abrasion_index])
                        E_abrasion[:] += (
                            abrasion_temp[:] *
        ...
        for x in range(constants.num_size):
            for tr2 in range(model_parameters.num_track):
                model_variables.M_road_bin_balance_data[s, :, constants.P_abrasion_index, ti, tr2, ro] += (
                    P_abrasion[x] * model_parameters.f_PM_bin[constants.abrasion_index, :, 0] *
                    model_parameters.f_track[tr2]
                )
            
            model_variables.E_road_bin_data[s, :, constants.E_direct_index, ti, tr, ro] += (
                E_abrasion[x] * model_parameters.f_PM_bin[constants.abrasion_index, :, 0]
            )
    ...
    model_variables.M_road_bin_balance_data[:, :, constants.P_crushing_index, ti, :, ro] = 0.0
    
    if model_flags.crushing_flag:
        for s in range(constants.num_source_all):
            if s < len(model_parameters.p_0_crushing) and model_parameters.p_0_crushing[s] > 0:
                for t in range(constants.num_tyre):
                    for v in range(constants.num_veh):
                        snow_ice_sum = np.sum(model_variables.g_road_data[constants.snow_ice_index, ti, tr, ro])
                        ...
                        R_crushing[s, :] += (
                            converted_data.traffic_data[constants.N_t_v_index[(t, v)], ti, ro] /
                            metadata.n_lanes * model_parameters.veh_track[tr] * f_crushing_temp
                        )
                        
                        model_variables.M_road_bin_balance_data[s, :, constants.S_crushing_index, ti, tr, ro] = (
                ...
                for x in range(constants.num_size - 1):
                    for x2 in range(x + 1, constants.num_size):
                        for tr2 in range(model_parameters.num_track):
                            model_variables.M_road_bin_balance_data[s, x2, constants.P_crushing_index, ti, tr2, ro] += (
                                model_variables.M_road_bin_balance_data[s, x, constants.S_crushing_index, ti, tr, ro] *
                                (1 - model_parameters.f_0_dir[constants.crushing_index] * 
                                 model_variables.f_q[s, ti, tr, ro]) *
                                model_parameters.f_PM_bin[constants.crushing_index, x2, 0] /
                                np.sum(model_parameters.f_PM_bin[constants.crushing_index, x+1:, 0])
                            )
                        
                        model_variables.E_road_bin_data[s, x2, constants.E_direct_index, ti, tr, ro] += (
                            model_variables.M_road_bin_balance_data[s, x, constants.S_crushing_index, ti, tr, ro] *
    ...
    if model_flags.dust_deposition_flag:
        if (len(airquality_data.PM_background) > ti and 
            airquality_data.PM_background[constants.pm_10, ti] != metadata.nodata):
            pm_bg_value = max(0, airquality_data.PM_background[constants.pm_10, ti])
        else:
            pm_bg_value = max(0, 20)  # Default value as in MATLAB
        
        model_variables.M_road_bin_balance_data[constants.depo_index, 1:, constants.P_depo_index, ti, tr, ro] = (
            ...
            pm_bg_value * 3.6 * metadata.b_road_lanes * model_parameters.f_track[tr]
        )
    else:
        model_variables.M_road_bin_balance_data[constants.depo_index, :, constants.P_depo_index, ti, tr, ro] = 0.0
    
    # --------------------------------------------------------------------------
    # Calculate road production due to sanding (P_sanding)
    # --------------------------------------------------------------------------
    model_variables.M_road_bin_balance_data[constants.sand_index, :, constants.P_depo_index, ti, tr, ro] = (
        converted_data.activity_data[constants.M_sanding_index, ti, ro] / time_config.dt *
    ...
    # --------------------------------------------------------------------------
    if ((airquality_data.EP_emis_available or metadata.exhaust_EF_available) and 
        model_flags.exhaust_flag):
        model_variables.M_road_bin_balance_data[constants.exhaust_index, :, constants.P_depo_index, ti, tr, ro] = (
            model_variables.E_road_bin_data[constants.exhaust_index, :, constants.E_total_index, ti, tr, ro] *
            model_parameters.f_PM_bin[constants.exhaust_index, :, 0] *
            model_parameters.f_track[tr] * (1 - model_parameters.f_0_dir[constants.exhaust_index])
        )
        model_variables.E_road_bin_data[constants.exhaust_index, :, constants.E_direct_index, ti, tr, ro] = (
            model_variables.E_road_bin_data[constants.exhaust_index, :, constants.E_total_index, ti, tr, ro] *
            model_parameters.f_PM_bin[constants.exhaust_index, :, 0] *
            model_parameters.f_track[tr] * model_parameters.f_0_dir[constants.exhaust_index]
        )
    else:
        model_variables.M_road_bin_balance_data[constants.exhaust_index, :, constants.P_depo_index, ti, tr, ro] = 0.0
        model_variables.E_road_bin_data[constants.exhaust_index, :, constants.E_direct_index, ti, tr, ro] = 0.0
    
    # --------------------------------------------------------------------------
    # Calculate road production due to fugitive deposition (P_fugitive g/km)
    # --------------------------------------------------------------------------
    model_variables.M_road_bin_balance_data[constants.fugitive_index, :, constants.P_depo_index, ti, tr, ro] = (
        (initial_data.P_fugitive + converted_data.activity_data[constants.M_fugitive_index, ti, ro]) /
        time_config.dt * model_parameters.f_PM_bin[constants.fugitive_index, :, 0] *
    ...
    for i in range(constants.num_salt):
        salt_flags = [model_flags.use_salting_data_1_flag, model_flags.use_salting_data_2_flag]
        model_variables.M_road_bin_balance_data[constants.salt_index[i], :, constants.P_depo_index, ti, tr, ro] = (
            converted_data.activity_data[constants.M_salting_index[i], ti, ro] / time_config.dt *
            model_parameters.f_PM_bin[constants.salt_index[i], :, 0] *
            1000 * metadata.b_road_lanes * model_parameters.f_track[tr] * salt_flags[i]
        )
    ...
    for s in range(constants.num_source_all):
        ...
        if s == constants.salt_index[0] and use_dissolved_ratio:
            not_dissolved_ratio_temp = (1.0 - 
                model_variables.road_salt_data[constants.dissolved_ratio_index, 0, ti, tr, ro])
        elif s == constants.salt_index[1] and use_dissolved_ratio:
            not_dissolved_ratio_temp = (1.0 - 
                model_variables.road_salt_data[constants.dissolved_ratio_index, 1, ti, tr, ro])
        
        for t in range(constants.num_tyre):
            for v in range(constants.num_veh):
                f_0_suspension_temp = (
                    model_parameters.h_0_sus[s, :] *
                    f_susroad_func(
                ...
                R_suspension_array[:] = (
                    converted_data.traffic_data[constants.N_t_v_index[(t, v)], ti, ro] /
                    metadata.n_lanes * model_parameters.veh_track[tr] * f_0_suspension_temp *
                    (model_variables.f_q[s, ti, tr, ro] * model_parameters.h_0_q_road[:] +
                     (1 - model_parameters.h_0_q_road[:])) *
                    not_dissolved_ratio_temp * model_flags.road_suspension_flag
                )
                R_suspension[s, :] += R_suspension_array[:]
        
        # Diagnose the suspension sink
        model_variables.M_road_bin_balance_data[s, :, constants.S_suspension_index, ti, tr, ro] = (
            R_suspension[s, :] * M_road_bin_0_data[s, :]
        )
        # Calculate the emissions. The same as the suspension sink
        model_variables.E_road_bin_data[s, :, constants.E_suspension_index, ti, tr, ro] = (
            model_variables.M_road_bin_balance_data[s, :, constants.S_suspension_index, ti, tr, ro]
        )
    ...
    for s in range(constants.num_source_all):
        ...
        R_windblown[s, constants.pm_sus] = (
            ...
            ) * model_variables.f_q[s, ti, tr, ro] * model_flags.wind_suspension_flag
        )
        
        model_variables.M_road_bin_balance_data[s, constants.pm_sus, constants.S_windblown_index, ti, tr, ro] = (
            R_windblown[s, constants.pm_sus] * M_road_bin_0_data[s, constants.pm_sus]
        )
        model_variables.E_road_bin_data[s, :, constants.E_windblown_index, ti, tr, ro] = (
            model_variables.M_road_bin_balance_data[s, :, constants.S_windblown_index, ti, tr, ro]
        )
    ...
    if (np.sum(model_variables.g_road_data[:constants.num_moisture, ti, tr, ro]) > 0 and 
        model_flags.dust_spray_flag):
        for s in range(constants.num_source_all):
            # Skip if source index is beyond the parameter arrays
            ...
            if s == constants.salt_index[0] and use_dissolved_ratio:
                dissolved_ratio_temp = model_variables.road_salt_data[constants.dissolved_ratio_index, 0, ti, tr, ro]
            elif s == constants.salt_index[1] and use_dissolved_ratio:
                dissolved_ratio_temp = model_variables.road_salt_data[constants.dissolved_ratio_index, 1, ti, tr, ro]
            else:
                dissolved_ratio_temp = 1.0
            
            h_eff_temp[0, :] = model_parameters.h_eff[constants.spraying_eff_index, s, :]
            R_spray[s, :] = (
                np.sum(model_variables.g_road_balance_data[:constants.num_moisture, constants.R_spray_index, ti, tr, ro]) *
                h_eff_temp[0, :] * dissolved_ratio_temp
            )
            
            model_variables.M_road_bin_balance_data[s, :, constants.S_dustspray_index, ti, tr, ro] = (
                R_spray[s, :] * M_road_bin_0_data[s, :]
            )
            
            # Production due to spray for multitracks
            model_variables.M_road_bin_balance_data[s, :, constants.P_dustspray_index, ti, tr, ro] = (
                np.sum(model_variables.g_road_balance_data[:constants.num_moisture, constants.P_spray_index, ti, tr, ro]) *
                h_eff_temp[0, :] * dissolved_ratio_temp * M_road_bin_0_data[s, :]
            )
    else:
        model_variables.M_road_bin_balance_data[:, :, constants.S_dustspray_index, ti, tr, ro] = 0.0
    ...
    if model_flags.drainage_type_flag == 1 or model_flags.drainage_type_flag == 3:
        ...
        if (model_variables.g_road_data[constants.snow_index, ti, tr, ro] < snow_limit and 
            model_flags.dust_drainage_flag > 0):
            for s in range(constants.num_source_all):
                # Skip if source index is beyond the parameter arrays
                if s >= model_parameters.h_eff.shape[1]:
                    continue
                ...
                R_drainage[s, :] = (
                    model_variables.g_road_balance_data[constants.water_index, constants.R_drainage_index, ti, tr, ro] *
                    model_parameters.h_eff[constants.drainage_eff_index, s, :] * dissolved_ratio_temp
                )
                
                model_variables.M_road_bin_balance_data[s, :, constants.S_dustdrainage_index, ti, tr, ro] = (
                    R_drainage[s, :] * M_road_bin_0_data[s, :]
                )
        else:
            model_variables.M_road_bin_balance_data[:, :, constants.S_dustdrainage_index, ti, tr, ro] = 0.0
    ...
    for s in range(constants.num_source_all):
        ...
        R_cleaning[s, :] = (
            -np.log(1 - np.minimum(0.99999, 
                model_parameters.h_eff[constants.cleaning_eff_index, s, :] *
                converted_data.activity_data[constants.t_cleaning_index, ti, ro])) /
            time_config.dt * model_flags.use_cleaning_data_flag
        )
        
        model_variables.M_road_bin_balance_data[s, :, constants.S_cleaning_index, ti, tr, ro] = (
    ...
    for s in range(constants.num_source_all):
        ...
        R_ploughing[s, :] = (
            ...
            time_config.dt * model_flags.use_ploughing_data_flag * model_flags.dust_ploughing_flag
        )
        
        model_variables.M_road_bin_balance_data[s, :, constants.S_dustploughing_index, ti, tr, ro] = (
    ...
    # --------------------------------------------------------------------------
    model_variables.M_road_bin_balance_data[:, :, constants.P_dusttotal_index, ti, tr, ro] = (
        model_variables.M_road_bin_balance_data[:, :, constants.P_wear_index, ti, tr, ro] +
        model_variables.M_road_bin_balance_data[:, :, constants.P_abrasion_index, ti, tr, ro] +
        model_variables.M_road_bin_balance_data[:, :, constants.P_crushing_index, ti, tr, ro] +
        model_variables.M_road_bin_balance_data[:, :, constants.P_depo_index, ti, tr, ro]
    )
    
    # --------------------------------------------------------------------------
    # Add up all the road sink rates (R_total)
    # --------------------------------------------------------------------------
    R_total = (R_drainage + R_cleaning + R_ploughing + R_spray + 
              R_crushing + R_suspension + R_windblown)
    ...
    for s in range(constants.num_source_all):
        for x in range(constants.num_size):
            model_variables.M_road_bin_data[s, x, ti, tr, ro] = mass_balance_func(
                ...
                model_variables.M_road_bin_balance_data[s, x, constants.P_dusttotal_index, ti, tr, ro],
                R_total[s, x],
                time_config.dt
            )
    
    # Diagnose sinks
    model_variables.M_road_bin_balance_data[:, :, constants.S_dusttotal_index, ti, tr, ro] = (
    ...
    if model_flags.drainage_type_flag == 2 or model_flags.drainage_type_flag == 3:
        ...
        drain_factor = (
            model_variables.g_road_balance_data[constants.water_index, constants.S_drainage_index, ti, tr, ro] *
            time_config.dt / (g_road_drainable_min + 
            model_variables.g_road_balance_data[constants.water_index, constants.S_drainage_index, ti, tr, ro] *
            time_config.dt)
        )
        ...
        for s in range(constants.num_source_all):
            ...
            model_variables.M_road_bin_balance_data[s, :, constants.S_dustdrainage_index, ti, tr, ro] = 0.0
            h_eff_temp[0, :] = model_parameters.h_eff[constants.drainage_eff_index, s, :]
            
            if model_flags.dust_drainage_flag == 1:
                model_variables.M_road_bin_balance_data[s, :, constants.S_dustdrainage_index, ti, tr, ro] = (
                    model_variables.M_road_bin_data[s, :, ti, tr, ro] *
                    dissolved_ratio_temp * h_eff_temp[0, :] * drain_factor / time_config.dt
                )
            elif model_flags.dust_drainage_flag == 2:
                model_variables.M_road_bin_balance_data[s, :, constants.S_dustdrainage_index, ti, tr, ro] = (
                    model_variables.M_road_bin_data[s, :, ti, tr, ro] *
                    dissolved_ratio_temp * (1 - np.exp(-h_eff_temp[0, :] * drain_factor)) / time_config.dt
                )
            ...
            model_variables.M_road_bin_balance_data[s, :, constants.S_dusttotal_index, ti, tr, ro] += (
                model_variables.M_road_bin_balance_data[s, :, constants.S_dustdrainage_index, ti, tr, ro]
            )
    ...
    model_variables.M_road_bin_balance_data[constants.total_dust_index, :, :, ti, tr, ro] = np.sum(
        model_variables.M_road_bin_balance_data[constants.dust_index, :, :, ti, tr, ro], axis=0
    )
    ...
    model_variables.E_road_bin_data[:, :, constants.E_total_index, ti, tr, ro] = (
        ...
        model_variables.E_road_bin_data[:, :, constants.E_suspension_index, ti, tr, ro] +
    ...
    # --------------------------------------------------------------------------
    model_variables.E_road_bin_data[constants.total_dust_index, :, :, ti, tr, ro] = np.sum(
        model_variables.E_road_bin_data[:constants.num_source, :, :, ti, tr, ro], axis=0
    )
